...
function AuthPageInner() {
  ...
  const handleLogin = async (e: React.SyntheticEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const redirect = role === "worker"
        ? "/provider/dashboard"
        : (nextUrl === "/" ? "/customer/dashboard" : nextUrl);
          ...
          {tab === "login" && (
            <form onSubmit={handleLogin} className="flex flex-col flex-1">
              ...
              <button
                ...
                disabled={loading}
                className="w-full flex items-center justify-center gap-2.5 border border-[var(--border-secondary)] bg-[var(--bg-primary)] text-[var(--text-primary)] py-2.5 rounded-lg text-sm hover:bg-[var(--bg-secondary)] transition-all disabled:opacity-50 cursor-pointer"
              >
